			...
			@Valid final AddToCartForm form, final BindingResult bindingErrors)
	{
		...
		else
		{
			try
			{
				...
				String productCode = cartModification.getEntry().getProduct().getCode();
				final ProductData productData = productFacade.getProductForCodeAndOptions(productCode, Arrays.asList(ProductOption.DESIGNER));
				OrderEntryData orderEntryData = cartModification.getEntry();
				if(productData.getDesigner() != null)
				{
				//	model.addAttribute("designer", productData.getDesigner());
					orderEntryData.getProduct().setDesigner(productData.getDesigner());
				}

				if (productData != null && productData.getDesigner() != null) {

					model.addAttribute("designer", productData.getDesigner().getName());
				}
				*/
